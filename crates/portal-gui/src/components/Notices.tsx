...
// uygulama tepside gizliyse) Rust sistem bildirimini gönderir — sistem toast'ı
// ekranı kesen bir jesttir, sen zaten bakarken atmanın anlamı yok.
...
import { onMonitorChanged } from "../lib/ipc";
...
export function Notices() {
  ...
  // yerden bakılır — DESTE AÇIKKEN durur. Kullanıcı okumak için açtıysa
  // şeritlerin altından kayması sinir bozucu olurdu.
  useEffect(() => {
    if (open) return;
    const t = setInterval(() => {
      const now = Date.now();
  ...
  const front = live[live.length - 1] ?? notes[notes.length - 1];
  ...
  return (
    <div className="notes">
      ...
      >
        ...
        <div className={"note note-front" + (front.leaving ? " leaving" : "")} role="status">
          <NoteBody
            note={front}
            extra={live.length - 1}
